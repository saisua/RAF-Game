...
class Attack_speed(Ability):
    ...
    tags = (Ability.tags | 
            {OFFENSIVE, MULTI_TURN, 
            FAST, STATUS, MULTI_TARGET}
        )
    name:str="attack speed boost"
    turns:int
    ...
    def start(self, combatter):
        ...
        self.previous_value = combatter.attributes.attack_speed
        
        combatter.attributes.attack_speed *= self.effectiveness

    def end(self, combatter):
        print(f"{self.name} has ended in {combatter.name}")
        combatter.attributes.attack_speed = self.previous_value